...
class App extends Component {
  ...
  getRandomDisplayImage = () => {
    const randomNumber = Math.floor(Math.random() * 30)
    const randomImageData = imagesList[randomNumber]

    return randomImageData
  }

  checkAnswer = id => {
    const {randomImageData} = this.state
    if (randomImageData.id === id) {
      const randomData = this.getRandomDisplayImage()

      this.setState(prevState => ({
        scoreCount: prevState.scoreCount + 1,
      }))
      this.setState({
        randomImageData: randomData,
      })
    } else {
  ...
  gameCard = () => {
    const {activeTab, randomImageData} = this.state
    const updatedImageList = this.getUpdatedList()
    return (
      <>
        ...
        <ul className="images-container">
          {updatedImageList.map(eachImage => (
            <ImageList
              key={eachImage.id}
    ...
    return (
      <div className="score-card-container">
        <div className="score-container">
          ...
          />
          <p>YOUR SCORE</p>
          <h1 className="final-score-count">{scoreCount}</h1>
          <div>
            <button
              type="button"
  ...
  render() {
    const {showScoreCard, scoreCount, sec} = this.state
          ...
          <ul className="nav-container">
            ...
            <li className="score-details-container">
              ...
              <div className="timer-details">
                ...
                />
                <p className="timer">{sec} sec</p>
              </div>
            </li>
          </ul>
          {showScoreCard ? this.showScore() : this.gameCard()}
        </div>
      </div>
    )
  }
